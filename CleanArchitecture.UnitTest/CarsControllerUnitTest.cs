    ...
    public class CarsControllerUnitTest
    {
        ...
        public async void Create_ReturnsOkResult_WhenRequestIsValid()
        {
            ...
            CreateCarCommand createCarCommand = new(
                "Fiat", "Linea", 2500);
            MessageResponse response = new("Success");
            CancellationToken cancellationToken = new();

            mediatorMock.Setup(x => x.Send(createCarCommand, cancellationToken)).ReturnsAsync(response);
            ...
            var returnValue = Assert.IsType<MessageResponse>(okResult);
